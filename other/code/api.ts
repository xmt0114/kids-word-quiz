...
export const API_CONFIG = {
  // 数据源配置：'local' 使用本地JSON，'backend' 使用后端API
  DATA_SOURCE: 'local', // 可以通过环境变量切换
...
class LocalWordAPI implements WordAPI {
  ...
  async addWord(word: any): Promise<WordApiResponse> {
    ...
    // 验证数据
    const errors = this.validateWord(word);
    if (errors.length > 0) {
      return {
        success: false,
  ...
  async updateWord(id: number, word: any): Promise<WordApiResponse> {
    ...
    // 验证数据
    const errors = this.validateWord({ ...word, id });
    if (errors.length > 0) {
      return {
        success: false,
        error: `数据验证失败: ${errors.join(', ')}`
  ...
  private validateWord(word: any): string[] {
    ...
    if (!word.audioText) errors.push('音频文本不能为空');
...
// 导出API实例
export const wordAPI: WordAPI = API_CONFIG.DATA_SOURCE === 'backend' 
